...
CREATE POLICY "Apenas admins podem criar jogadores" ON users
  FOR INSERT TO authenticated WITH CHECK (
    EXISTS (
      SELECT 1 FROM users WHERE users.email = (SELECT email FROM auth.users WHERE auth.users.id = auth.uid()) AND users.perfil = 'admin'
    )
  );

CREATE POLICY "Apenas admins podem atualizar outros jogadores" ON users
  FOR UPDATE TO authenticated USING (
    users.email = (SELECT email FROM auth.users WHERE auth.users.id = auth.uid()) OR
    EXISTS (
      SELECT 1 FROM users WHERE users.email = (SELECT email FROM auth.users WHERE auth.users.id = auth.uid()) AND users.perfil = 'admin'
    )
  );
...
CREATE POLICY "Usuários podem confirmar sua própria presença" ON presencas
  FOR INSERT TO authenticated WITH CHECK (
    EXISTS (
      SELECT 1 FROM users 
      WHERE users.id = presencas.user_id 
